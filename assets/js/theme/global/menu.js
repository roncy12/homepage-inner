...
class Menu {
    ...
    onMenuClick(event) {
        ...
        if (this.hasMaxMenuDisplayDepth) {
            const $neighbors = $(event.target).parents('.navPages li').siblings();

            this.collapseNeighbors($neighbors);
        }

        const $marketplaceContainer = $(event.target).parents('.marketplace--enclosed');
        const $marketplacePopout = $(event.target).closest('li').find('.marketplace-popout:visible');

        if ($marketplaceContainer.length > 0 && $marketplacePopout.length > 0) {
            $(event.target)
                .parents('#navPages-main')
                .css('min-height', $marketplacePopout.outerHeight());
        }
    }
